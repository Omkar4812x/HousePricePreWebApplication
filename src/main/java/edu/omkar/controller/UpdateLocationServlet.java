...
public class UpdateLocationServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<h2 class='text-center mb-4'>Update Location</h2>");

        out.println("<input type='hidden' name='id' value='" + locId + "'>");
        ...
        for (CityModel cm : cities) {
            String sel = (cm.getId() == ctid) ? "selected" : "";
            out.println("<option value='" + cm.getId() + "' " + sel + ">" + cm.getName() + " (" + cm.getStateName() + ")</option>");
        }
        out.println("</select></div>");

        out.println("<div class='mb-4'><label class='form-label'>Location Name</label>");
        out.println("<input type='text' name='name' value='" + locName + "' class='form-control form-control-lg' required></div>");

        out.println("<div class='d-grid'><input type='submit' value='Update Location' name='s' class='btn btn-warning btn-lg'></div>");
        out.println("</div></div></div></div></div></form>");

        String btn = request.getParameter("s");
        if (btn != null) {
            LocationModel lm = new LocationModel();
            lm.setId(Integer.parseInt(request.getParameter("id")));
            lm.setName(request.getParameter("name"));
            lm.setCityId(Integer.parseInt(request.getParameter("ctid")));
            LocationService locationService = new LocationServiceImpl();
            boolean b = locationService.updateLocation(lm);
            if (b) response.sendRedirect("viewlocation");
            else   out.println("<h4 class='text-danger mt-3'>Location Not Updated.</h4>");
        }
    }
