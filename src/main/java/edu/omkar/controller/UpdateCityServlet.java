...
public class UpdateCityServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<h2 class='text-center mb-4'>Update City</h2>");

        out.println("<input type='hidden' name='id' value='" + cityId + "'>");
        out.println("<input type='hidden' name='stateid_hidden' value='" + stateId + "'>");
        ...
        for (StateModel sm : states) {
            String sel = (sm.getId() == stateId) ? "selected" : "";
        ...
        out.println("<input type='text' name='name' value='" + cityName + "' class='form-control form-control-lg' required></div>");

        out.println("<div class='d-grid'><input type='submit' value='Update City' name='s' class='btn btn-warning btn-lg'></div>");
        ...
        if (btn != null) {
            ...
            boolean b = cityService.updateCity(cm);
            if (b) response.sendRedirect("viewcity");
            else   out.println("<h4 class='text-danger mt-3'>City Not Updated. Try again.</h4>");
        }
    }
